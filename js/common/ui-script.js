...
//     const gnbMenu = document.querySelector(".krds-main-menu");
...
const krds_accordion = {
  ...
  setupAllToggleButton(){
	...
	allToggleBtns.forEach(btn => {
		btn.addEventListener("click", ()=>{
			...
			if(!targetSelector) return; // target 없으면 실행 안 함
			
			// targetSelector에 포함된 모든 컨테이너 내의 .accordion-item 한 번에 수집
			const combinedSelector = targetSelector.split(',').map(s => `${s.trim()} .accordion-item`).join(',');
  ...
  setItemState(item, forceState){
	const button = item.querySelector(".btn-accordion");
	if (!button) return;
...
const krds_modal = {
  ...
  setupTriggers() {
    ...
    this.modalOpenTriggers.forEach((trigger) => {
      const eventAt = trigger.dataset.open;
      if(eventAt != 'on'){
        trigger.dataset.open = 'on';
    ...
    this.modalCloseTriggers.forEach((trigger) => {
      const eventAt = trigger.dataset.close;
      if(eventAt != 'on'){
        trigger.dataset.close = 'on';
        trigger.addEventListener("click", (event) => {
          event.preventDefault();
  ...
  openModal(id) {
	...
	const modalCloseBtn = modalElement.querySelector(".close-modal");
...
/*** * alert modal * ***/
const alertModal = {
	open(title, content, triggerElId) {
		/*
			param title = 알럿 제목
			param content = 알럿 내용
			triggerElId = 모달 닫았을 때 포커스 될 요소 id
		 */
		const modalId = 'alert-modal';
		const modalEl = document.getElementById(modalId);
		
		modalEl.querySelector('#alert-modal-title').innerText = title;
		modalEl.querySelector('#alert-modal-content').innerText = content;
		
		if (triggerElId) {
			const triggerBtn = document.getElementById(triggerElId);
			
			if (triggerBtn){
				triggerBtn.setAttribute('data-modal-id', modalId);
				triggerBtn.classList.add('modal-opened');
			}
		}
		
		krds_modal.openModal(modalId);
	}	
}
